...
class Lab2:
    ...
    def handleRobotState(self, msg):
        """"
        Stores the last published robot state in a variable so that it can accessed any time
        """
        self.robotState = msg.data

    def spin(self):
        """""
        Spins the robot until it is localized ie: the covariance drops belows a certain variable
        """
    ...
    def update_path(self, msg):
        """
        Cancels any existing path that is running and restarts with given path
        Takes in msg type "Path"
        """
    ...
    def drive_path(self,msg):
        """
        Makes the robot follow a type Path message
        If the robot is in the exploration state it will not drive the path fully. However it will always try to drive the robot atleast 10 cm
        """
        rospy.sleep(0.11)
        dist = 0

        ##In exploration, will not complete full path
        if(self.robotState =="Exploration"):
            
            driveList = [] # Path of points to drive to
            point1 = msg.poses[0] # set initial pose
            
            # If first waypoint is less than 10 cm away. Add waypoints until the robot covers a distance greater than 10 cm
            if(math.sqrt(math.pow(point1.pose.position.x-self.px,2) + math.pow(point1.pose.position.y-self.py,2))<0.1):

                ##For each index, go through the poses and add to
                for index, point in enumerate(msg.poses):
                    point1 = msg.poses[index]
                    if(math.sqrt(math.pow(point1.pose.position.x-self.px,2) + math.pow(point1.pose.position.y-self.py,2))<0.2 and math.sqrt(math.pow(point1.pose.position.x-self.px,2) + math.pow(point1.pose.position.y-self.py,2))>0.05):
                        driveList.append(point1)
                    # dist=math.sqrt(math.pow(point1.pose.position.x-self.px,2) + math.pow(point1.pose.position.y-self.py,2))
                    # rospy.loginfo(dist)
                    # if(dist>0.05):
                    #     driveList.append(point1)
                    ...
                    #     dist += math.sqrt(math.pow(point1.pose.position.x-point2.pose.position.x,2) + math.pow(point1.pose.position.y-point2.pose.position.y,2))
                    #     if(dist >0.1):
                    #         break
                    #     driveList.append(point1)
        ...
        # If the state is not exploring, drive the whole path
